...
import com.example.board.model.shopping.ShoppingResponse;
import com.example.board.model.shopping.ShoppingResponse.Body.Items.Item;
...
public class S_ApiService {
    ...
    ShoppingResponse apiResponse = new ShoppingResponse();
    try {
        JAXBContext jaxbContext = JAXBContext.newInstance(ShoppingResponse.class); // JAXB Context 생성
        Unmarshaller unmarshaller = jaxbContext.createUnmarshaller();  // Unmarshaller Object 생성
        apiResponse = (ShoppingResponse) unmarshaller.unmarshal(new StringReader(xml)); // unmarshall 메서드 호출
