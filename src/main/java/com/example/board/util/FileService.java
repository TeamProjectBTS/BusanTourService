...
public class FileService {
    ...
    public AttachedFile saveFile(MultipartFile mfile) {
        ...
        String savedFilename = originalFilename + sdf.format(new Date().getTime());
    ...
    public ReviewAttachedFile saveReviewFile(MultipartFile mfile) {
      // 업로드 된 파일이 없거나 크기가 0이면 저장하지 않고 null을 리턴
      if (mfile == null || mfile.isEmpty() || mfile.getSize() == 0) {
          return null;
      }
      ...
      return new ReviewAttachedFile(originalFilename, savedFilename + ext, mfile.getSize());
  }
   ...
   */
    ...
    public MemberAttachedFile profileAttachedFile(MultipartFile mfile) {
        // 업로드 된 파일이 없거나 크기가 0이면 저장하지 않고 null을 리턴
        if (mfile == null || mfile.isEmpty() || mfile.getSize() == 0) {
            return null;
        }
